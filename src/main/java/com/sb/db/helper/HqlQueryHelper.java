...
public class HqlQueryHelper {
    ...
    public static List<Project> getProjects(Session session, long organizationId) {
        List<Project> projects = session.createQuery("from Project where organization.id ='"+ organizationId +"'").list();
		return projects;
    }
    
    public static List<UserStory> getUserStorys(Session session, long iterationId) {
        List<UserStory> userStories = session.createQuery("from UserStory where iteration.iterationId ='"+ iterationId +"'").list();
